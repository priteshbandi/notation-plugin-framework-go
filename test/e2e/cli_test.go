...
func TestSuccess(t *testing.T) {
	tests := map[string]struct {
		...
		"verify-signature": {
			stdin:          "",
			expectedStdout: "{\"name\":\"com.example.plugin\",\"description\":\"This is an description of example plugin\",\"version\":\"1.0.0\",\"url\":\"https://example.com/notation/plugin\",\"capabilities\":[\"SIGNATURE_GENERATOR.ENVELOPE\",\"SIGNATURE_VERIFIER.TRUSTED_IDENTITY\",\"SIGNATURE_VERIFIER.REVOCATION_CHECK\"]}",
		},
		"version": {
			stdin:          "",
...
func validateSuccess(arg, stdInput, expectedStdOut string, t *testing.T) {
	...
	if jsonEquals(stdOut, expectedStdOut) {
...
func validateFailure(arg, stdInput, expectedStdError string, t *testing.T) {
	...
	if err == nil {
		t.Errorf("expected '%s' command fail with error but it didnt", arg)
		t.Logf("standard output: %s", stdOut)
		t.Logf("standard Err: %s", stdErr)
	}
...
func jsonEquals(x, y string) bool {
	var x1, y1 interface{}
